...
export class ProductService {
 ...
 getProducts(): Observable<Product[]> {
  ...
  modifierProduct(productId: string, product: Product): Observable<any> {
    const updateProductUrl = `${this.apiUrl}/products/${productId}`;
    return this.http.put(updateProductUrl, product);
  }


  deleteProduct(productId: number): Observable<any> {
    const deleteProductUrl = `${this.apiUrl}/products/${productId}`;
